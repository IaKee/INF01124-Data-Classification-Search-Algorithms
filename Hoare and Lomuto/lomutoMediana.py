...
def quickSort(array, primeiroTermo, ultimoTermo):
    ...
    if primeiroTermo < ultimoTermo:
        ...
        recursoes += 1
        quickSort(array, primeiroTermo, pontodeQuebra - 1)
        recursoes += 1
        quickSort(array, pontodeQuebra + 1, ultimoTermo)
def particionaLomuto(array, primeiroTermo, ultimoTermo, particionador):
    indiceMenorTermo = primeiroTermo - 1
    for number in range(primeiroTermo, ultimoTermo):
        if array[number] <= particionador:
            swap(array[indiceMenorTermo], array[number])
            indiceMenorTermo += 1
    swap(array[indiceMenorTermo+1], array[ultimoTermo])
    return indiceMenorTermo+1


with open("entrada-quicksort.txt", 'r') as inputFile:
    with open("stats-mediana-lomuto.txt", 'w') as outputFile:
